...
function TodoList({appTodo}){
    ...
    return (
      <>
        ...
        {/* {storeTodo.map((todo)=>( */}
          <li
            className="mt-4 flex justify-between items-center bg-zinc-800 px-4 py-2 rounded w-2/4 m-auto"
            key={appTodo.id}
          >
            {/* <div className="text-white">{appTodo.text}</div> */}
            <input 
            // className="text-white bg-transparent" 
            className={`border outline-none w-9/12 bg-transparent text-white rounded-lg py-2 ${
              isTodoEditable ? "border-white/10 px-2" : "border-transparent"
            ...
            <button onClick={deleteBtn} className="text-white bg-red-500 border-0 py-1 px-4 focus:outline-none hover:bg-red-600 rounded text-md ml-1">
              <svg
